...
def _squeeze(inputs, hidden_channel, ratio, block_id, sub_block_id):
    ...
    # x = Reshape((1, 1, -1))(x)    # 这句报错，是tensorflow版本的问题。解决请参考：  https://blog.csdn.net/DD_PP_JJ/article/details/115622764?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522165102704616780357279531%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fall.%2522%257D&request_id=165102704616780357279531&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-115622764-null-null-2~all~first_rank_ecpm_v1~rank_v31_ecpm-1-115622764.142^v9^pc_search_result_cache,157^v4^control&utm_term=The+channel+dimension+of+the+inputs+should+be+defined.+Found+%60None%60.++++ghostnet&spm=1018.2226.3001.4187
    b = x.shape[1]
    x = Reshape((1, 1, b))(x)
...
def _ghost_module(inputs, exp, ratio, block_id, sub_block_id, part, kernel_size=1, dw_size=3, stride=1, relu=True):
    ...
    x = Conv2D(output_channels, kernel_size, strides=stride, padding="same", use_bias=False,
               kernel_initializer=RandomNormal(stddev=0.02),
               name="blocks." + str(block_id) + "." + str(sub_block_id) + ".ghost" + str(part) + ".primary_conv.0")(
        inputs)
    x = BatchNormalization(
        name="blocks." + str(block_id) + "." + str(sub_block_id) + ".ghost" + str(part) + ".primary_conv.1")(x)
    if relu:
        x = Activation('relu')(x)
    ...
    #   跨特征点的特征提取
    # --------------------------------------------------------------------------#
    dw = DepthwiseConv2D(dw_size, 1, padding="same", depth_multiplier=ratio - 1, use_bias=False,
                         depthwise_initializer=RandomNormal(stddev=0.02),
                         name="blocks." + str(block_id) + "." + str(sub_block_id) + ".ghost" + str(
                             part) + ".cheap_operation.0")(x)
    dw = BatchNormalization(
        name="blocks." + str(block_id) + "." + str(sub_block_id) + ".ghost" + str(part) + ".cheap_operation.1")(dw)
    if relu:
...
def Ghostnet(inputs):
    ...
    x = Conv2D(16, (3, 3), padding="same", strides=(2, 2), use_bias=False, kernel_initializer=RandomNormal(stddev=0.02),
               name="conv_stem")(inputs)
